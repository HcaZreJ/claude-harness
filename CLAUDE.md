...
1. **本次改动即将写入第 5 个文件**（`block-unplanned-sprawl.sh` 在工具层判定并提示；阈值由 `SPRAWL_MAX_FILES` 调整）
2. 触及 schema、migration、数据模型
3. 触及 auth、权限、计费、判分
4. 不可逆数据操作
5. 改动共享或全局配置：harness、CI workflow、多 repo 复用的 prompt 或库
6. 同步请求路径新增外部调用，或新增后台任务与并发结构
7. 用户说了 plan、设计、规划、重构、架构
